 ...
 *
 * The **name** is not here: that one belongs to the player rather than to
 * Skyjo, and lives in {@link @/online/player-name} where every game finds it.
 *
 * Stored per browser under a Skyjo-specific key. The count is clamped on the
 * way out, so a hand-edited or outdated entry can never ask for a table the
 * game cannot seat.
 */
