/**
 * Exploding Kittens settings: how many sit at the table, and how long it lasts.
 *
 * @module
 */
"use client";
...
import { PLAYER_COUNTS } from "@/games/exploding-kittens/settings/app-settings";
import {
 ...
 */
export function ExplodingKittensSettingsView(): ReactElement {
  ...
  return (
    <div className="mx-auto flex w-full max-w-2xl flex-1 flex-col gap-5 p-4">
        ...
        >
          {PLAYER_COUNTS.map((count) => (
            <button
              ...
              data-testid={`exploding-kittens-players-${count}`}
              onClick={() =>
      ...
      <section className="flex flex-col gap-2 rounded-2xl border border-zinc-200 p-4 dark:border-zinc-800">
        <div>
          <h2 className="text-sm font-semibold">{T.fastGame}</h2>
          <p className="text-xs text-zinc-500 dark:text-zinc-400">
            {T.fastGameHint}
          </p>
        </div>
        ...
        >
          {[false, true].map((option) => (
            <button
              ...
              type="button"
              role="radio"
              aria-checked={option === settings.fastGame}
              data-testid={`exploding-kittens-fast-${option ? "on" : "off"}`}
              onClick={() => updateSettings({ ...settings, fastGame: option })}
              className={`cursor-pointer rounded-lg border px-4 py-2 text-sm font-semibold ${
                option === settings.fastGame
                  ? "border-indigo-500 bg-indigo-600 text-white"
                  : "border-zinc-300 hover:bg-zinc-100 dark:border-zinc-700 dark:hover:bg-zinc-800"
              }`}
            >
              {option ? T.fastOn : T.fastOff}
            </button>
          ))}
        </div>
      </section>
