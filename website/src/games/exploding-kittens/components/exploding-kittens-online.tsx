/**
 * Exploding Kittens online: the entry screen, the lobby and the table.
 *
 * @module
 * @remarks
 ...
 * it hands back.
 *
 * The one thing this game adds to the entry screen is a choice: the rulebook's
 * faster game. It rides in the shared wish's `variant` slot, so automatic
 * matchmaking only ever puts together people who asked for the same length of
 * game - being dropped into a shorter one than you signed up for is the sort of
 * surprise that ends an evening.
 *
...
import { explodingKittensAdapter } from "@/games/exploding-kittens/multiplayer/adapter";
import type { ExplodingKittensOptions } from "@/games/exploding-kittens/engine/setup";
import {
  DEFAULT_PLAYER_COUNT,
  PLAYER_COUNTS,
} from "@/games/exploding-kittens/settings/app-settings";
import {
  loadMatchCount,
  loadMatchFast,
  saveMatchCount,
  saveMatchFast,
} from "@/games/exploding-kittens/settings/online-settings";
...
const L = {
  ...
  subtitle: "Zieh keine Bombe - wer als Letzter lebt, gewinnt",
  back: "Zurück",
  yourName: "Dein Name",
  ...
  gameLength: "Spiel:",
  fullGame: "Ganzes Deck",
  fastGame: "Schnell",
  fastHint:
    "Beim schnellen Spiel fehlt ein Drittel des Nachziehstapels - kürzere Partie.",
  hostChooses: "Der Host entscheidet, welches Spiel gespielt wird.",
} as const;
...
const GAME_ID = explodingKittensAdapter.gameId;

/**
 * How long a player may dither before the computer plays their turn.
 *
 * @remarks
 * Every decision here is small - play something or draw, nope it or do not -
 * but there are a lot of them, and one of them is asked of everybody at once.
 * Short enough that a table is never held up by somebody who has wandered off.
 */
...
/** The name the faster game goes by in the shared wish. */
const FAST_VARIANT = "fast";

/** The name the plain game goes by in the shared wish. */
const FULL_VARIANT = "full";

/**
 * The wish advertised while searching.
 *
 * @param count - the wished table size
 * @param fast - whether the faster game is wanted
 * @returns the wish two searchers must agree on to be put together
 * @remarks
 * The deck flags belong to Drecksau and stay false here. How long the game
 * should be goes in the game-specific `variant` slot, which is what keeps a
 * table from being assembled out of people who wanted two different games.
 */
function matchWish(count: number, fast: boolean): Wish {
  return {
    count,
    expansion: false,
    defense: false,
    variant: fast ? FAST_VARIANT : FULL_VARIANT,
  };
}
 ...
 */
export function ExplodingKittensOnlineScreen(): ReactElement {
  ...
  // Which of the two games everybody at this table is here for.
  const [fast, setFast] = useState(false);
  const room = useOnlineRoom(explodingKittensAdapter, session);
  ...
  const startAuto = useCallback(
    async (name: string, count: number, wantsFast: boolean) => {
      setWanted(count);
      setFast(wantsFast);
      const found = await findMatch(
        database(),
        GAME_ID,
        matchWish(count, wantsFast),
        Date.now(),
      );
      setAuto(found);
  ...
  const startPrivate = useCallback(
    (next: OnlineSession, wantsFast: boolean) => {
      setFast(wantsFast);
      setAuto(null);
      setSession(next);
    },
...
function Entry({
  ...
  readonly onStart: (session: OnlineSession, fast: boolean) => void;
  ...
  const chooseFast = (next: boolean) => {
    setFast(next);
    saveMatchFast(next);
  };
  const host = () => {
    remember();
    onStart({ mode: "host", code: generateRoomCode(), name }, fast);
  };
  const join = () => {
    const clean = normalizeRoomCode(code);
    if (isValidRoomCode(clean)) {
      remember();
      // A guest plays whatever the host deals. The flag rides along only so the
      // screen knows which game to name while everyone is still waiting.
      onStart({ mode: "guest", code: clean, name }, fast);
    }
  };
  const autoMatch = () => {
    remember();
    ...
    void onAutoMatch(name, count, fast).catch(() => setSearching(false));
  };
  ...
  return (
    <div className="flex max-w-md flex-col gap-5">
      ...
      <div className="flex flex-col gap-3 rounded-2xl border border-emerald-200 bg-emerald-50/40 p-4 dark:border-emerald-900/50 dark:bg-emerald-950/20">
        ...
        <FastPicker fast={fast} onChoose={chooseFast} />
        <button
          type="button"
          onClick={autoMatch}
          disabled={searching}
...
/** Picks which of the two games this table is playing. */
function FastPicker({
  fast,
  onChoose,
}: {
  readonly fast: boolean;
  readonly onChoose: (fast: boolean) => void;
}): ReactElement {
  return (
    <div className="flex flex-col gap-1">
      <div
        role="radiogroup"
        aria-label={L.gameLength}
        className="flex flex-wrap items-center gap-2"
      >
        <span className="text-xs text-zinc-500 dark:text-zinc-400">
          {L.gameLength}
        ...
        {[false, true].map((option) => (
          <button
            ...
            type="button"
            role="radio"
            aria-checked={option === fast}
            data-testid={`ek-fast-${option ? "on" : "off"}`}
            onClick={() => onChoose(option)}
            className={
              option === fast
                ? "cursor-pointer rounded-lg border border-emerald-500 bg-emerald-100 px-3 py-1 text-sm font-medium text-emerald-800 dark:bg-emerald-900/50 dark:text-emerald-200"
...
function SearchingLobby({
  ...
  /** Whether this searcher asked for the faster game. */
  readonly fast: boolean;
  readonly onlineCount: number | null;
  ...
  const startNow = useCallback(() => {
    ...
    start({ fastGame: fast, autoPlayMs: AUTO_PLAY_MS });
  }, [match.code, start, fast]);
  ...
  useEffect(() => {
    ...
    return () => clearInterval(timer);
  }, [isHost, here, match.code, onHop, wanted, relaxed, fast]);
    ...
    <div className="flex max-w-md flex-col gap-5">
      ...
      <section className="flex flex-col items-center gap-3 rounded-2xl border border-zinc-200 p-6 text-center dark:border-zinc-800">
        ...
        <p className="text-sm text-zinc-600 dark:text-zinc-300">
          {L.playersHere(here, wanted)} · {fast ? L.fastGame : L.fullGame}
        </p>
...
function Lobby({
  ...
  /** The game the host has settled on. */
  readonly fast: boolean;
  readonly onFast: (fast: boolean) => void;
  ...
  return (
    <div className="flex max-w-md flex-col gap-5">
      ...
      {room.isHost ? (
        <>
          <FastPicker fast={fast} onChoose={onFast} />
          <button
            type="button"
            disabled={!enough}
            onClick={() =>
              room.start({ fastGame: fast, autoPlayMs: AUTO_PLAY_MS })
            }
...
function Playing({
  ...
  const remainingMs = useTurnClock({
    ...
    turn:
      room.room === null ? "" : turnKeyOf(room.room, explodingKittensAdapter),
