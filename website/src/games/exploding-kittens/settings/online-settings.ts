...
const COUNT_KEY = storageKey("exploding-kittens", "online-player-count");

/** Storage key for whether the faster game is wanted online. */
const FAST_KEY = storageKey("exploding-kittens", "online-fast-game");
...
/**
 * Loads whether this player wants the faster game when searching.
 *
 * @returns true if the reduced deck was chosen last time
 */
export function loadMatchFast(): boolean {
  return readStored(FAST_KEY, ONLINE_VERSION, isBoolean) === true;
}

/**
 * Stores whether the faster game is wanted.
 *
 * @param fast - what the player chose
 */
export function saveMatchFast(fast: boolean): void {
  writeStored(FAST_KEY, ONLINE_VERSION, fast);
}

/** Whether a stored value is a usable flag. */
