/**
 * Jammerlappen online: the entry screen, the lobby and the table.
 *
 * @module
 * @remarks
 ...
 * it hands back.
 *
 * It needs no loop of its own even though one of its moves is a race. A
 * Zwischenschmeiss is simply a move sent by a seat whose turn it is not, and
 * the host is the referee - the first one to arrive is the one that lands, and
 * every other client is told about it in the same snapshot as everything else.
 *
...
import {
  jammerlappenAdapter,
  type JammerlappenOptions,
} from "@/games/jammerlappen/multiplayer/adapter";
import {
  DEFAULT_PLAYER_COUNT,
  PLAYER_COUNTS,
} from "@/games/jammerlappen/settings/app-settings";
import {
  loadMatchCount,
  saveMatchCount,
} from "@/games/jammerlappen/settings/online-settings";
...
const L = {
  ...
  subtitle: "Werd deine Karten los - der Letzte ist der Jammerlappen",
  back: "Zurück",
  yourName: "Dein Name",
...
const GAME_ID = jammerlappenAdapter.gameId;

/**
 * How long a player may dither before the computer plays their turn.
 *
 * @remarks
 * On the short side, and for the game's own sake: a pot that has been sitting
 * there for a minute is a pot nobody is racing anybody for. Long enough to look
 * at a hand of three, short enough that the table keeps moving.
 */
 ...
 */
export function JammerlappenOnlineScreen(): ReactElement {
  ...
  const room = useOnlineRoom(jammerlappenAdapter, session);
    ...
    <div className="mx-auto flex w-full max-w-6xl flex-1 flex-col gap-4 p-4">
      <header className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <RulesButton rules={JAMMER_RULES} />
          <Link
            href="/jammerlappen"
...
type Room = OnlineRoom<JammerlappenGame, JammerlappenMove, JammerlappenOptions>;
...
function Playing({
  ...
  const remainingMs = useTurnClock({
    ...
    turn: room.room === null ? "" : turnKeyOf(room.room, jammerlappenAdapter),
