/**
 * Jammerlappen settings, stored in the browser.
 *
 * @module
 * @remarks
 ...
 */
import { MAX_PLAYERS, MIN_PLAYERS } from "@/games/jammerlappen/engine/state";
...
const SETTINGS_KEY = storageKey("jammerlappen", "settings");

/**
 * Table size of a first-time visitor's game.
 *
 * @remarks
 * Four, not two. Two is a game of its own here - it is played with a reduced
 * deck and triplets instead of quartets - and the game most people mean by
 * Jammerlappen is the one where the turn comes round a table.
 */
...
export type JammerlappenSettings = {
  /** How many sit at the table, the human included. */
  readonly playerCount: number;
};

/** What a first-time visitor gets. */
export const DEFAULT_SETTINGS: JammerlappenSettings = {
 ...
 */
export function loadSettings(): JammerlappenSettings {
 ...
 */
export function saveSettings(settings: JammerlappenSettings): void {
...
function isPartialSettings(
  value: unknown,
): value is Partial<JammerlappenSettings> {
