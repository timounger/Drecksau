/**
 * Bohnanza settings, stored in the browser.
 *
 * @module
 * @remarks
 ...
 */
import { MAX_PLAYERS, MIN_PLAYERS } from "@/games/bohnanza/engine/state";
...
const SETTINGS_KEY = storageKey("bohnanza", "settings");

/**
 * Table size of a first-time visitor's game.
 *
 * @remarks
 * Four, which is also the table the rulebook draws its examples on. It is the
 * smaller board - two fields instead of three - and that is the version of
 * Bohnanza people mean: with three fields there is room for whatever turns up,
 * and the whole game is about there not being room.
 */
 ...
 */
export function loadSettings(): BohnanzaSettings {
 ...
 */
export function saveSettings(settings: BohnanzaSettings): void {
...
function isPartialSettings(value: unknown): value is Partial<BohnanzaSettings> {
