...
import {
  arschlochAdapter,
  type ArschlochOptions,
} from "@/games/arschloch/multiplayer/adapter";
import {
...
import {
  loadMatchCount,
  saveMatchCount,
} from "@/games/arschloch/settings/online-settings";
import { ARSCHLOCH_TEXTS as GT } from "@/games/arschloch/i18n/texts";
...
import { loadSettings } from "@/games/arschloch/settings/app-settings";
...
const L = {
  title: `${GT.title} online`,
  subtitle: "Karten loswerden - jede:r am eigenen Gerät",
  back: "Zurück",
  yourName: "Dein Name",
...
const GAME_ID = arschlochAdapter.gameId;
 ...
 */
export function ArschlochOnlineScreen(): ReactElement {
  ...
  const room = useOnlineRoom(arschlochAdapter, session);
...
type Room = OnlineRoom<ArschlochGame, ArschlochMove, ArschlochOptions>;
...
function SearchingLobby({
  ...
  const startNow = useCallback(() => {
    ...
    start({ autoPlayMs: AUTO_PLAY_MS, rounds: loadSettings().rounds });
  }, [match.code, start]);
...
function Playing({
  ...
  const remainingMs = useTurnClock({
    ...
    turn: room.room === null ? "" : turnKeyOf(room.room, arschlochAdapter),
    ...
    <div className="flex flex-col gap-4 lg:flex-row">
      <div className="flex flex-1 flex-col gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <span data-testid="ar-turn" className="text-sm font-semibold">
            {game.phase === "gameOver"
              ? GT.gameOver
              : game.phase === "roundOver"
                ? GT.roundOver
                : seatOnTurn(game) === mySeat
                  ? GT.yourTurn
                  : GT.waitingFor(
                      game.players[seatOnTurn(game) ?? 0]?.name ?? "",
                    )}
          </span>
          <span className="text-xs text-zinc-500 dark:text-zinc-400">
            {GT.round(game.round, game.rounds)}
          </span>
