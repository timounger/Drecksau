/**
 * Dog online: the entry screen, the lobby and the table.
 *
 * @module
 * @remarks
 * The same shared layer as every other game here: {@link useOnlineRoom} does
 * the room, the host election and the redaction, and this screen only renders
 * what it hands back.
 *
 * Two ways in, like the other games: automatic matchmaking against strangers
 * (with Dog that is always a table of four - it is a partner game, and three
 * people cannot play it) or a private room whose four-letter code you pass
 * around. Who sits where decides the teams: seats one and three play together,
 * seats two and four against them, exactly as at a table.
 */
"use client";
...
import { dogAdapter, type DogOptions } from "@/games/dog/multiplayer/adapter";
import {
...
import {
  loadMatchCount,
  saveMatchCount,
} from "@/games/dog/settings/online-settings";
...
const L = {
  ...
  subtitle: "Zu zweit gegen zwei - jede:r am eigenen Gerät",
  back: "Zurück",
  yourName: "Dein Name",
  namePlaceholder: "z. B. Alex",
  autoTitle: "Automatisch matchen",
  autoHint:
    "Passt ein offener Tisch, geht es sofort los - sonst nach kurzer Wartezeit auch mit einer anderen Spielerzahl. Zu viert und zu sechst spielt ihr in Teams, sonst jeder für sich.",
  ...
  relaxedNow: "Wir starten jetzt auch mit einer anderen Spielerzahl.",
  botSeats: (n: number) =>
    n === 1
      ? "Ein Platz wird vom Computer gespielt"
      : `${n} Plätze werden vom Computer gespielt`,
...
const GAME_ID = dogAdapter.gameId;
 ...
 */
export function DogOnlineScreen(): ReactElement {
  ...
  const room = useOnlineRoom(dogAdapter, session);
...
type Room = OnlineRoom<DogGame, DogMove, DogOptions>;
...
function Playing({
  ...
  const onTurn = game === null ? null : seatOnTurn(game);
  const remainingMs = useTurnClock({
    autoPlayMs: room.room?.autoPlayMs ?? null,
    turn: room.room === null ? "" : turnKeyOf(room.room, dogAdapter),
      ...
      <div className="flex flex-1 flex-col gap-4">
        {game.phase === "gameOver" && (
          <>
            ...
            <section className="flex flex-wrap items-center gap-3 rounded-2xl border border-emerald-300 bg-emerald-50 p-4 dark:border-emerald-800 dark:bg-emerald-950/40">
        ...
        {/* The same play area as the game against the computer: one board, one
            hand, one list of what a card can do. A seat that is not at this
            table gets no hand, which is what a spectator sees. */}
        <PlayArea
          game={game}
