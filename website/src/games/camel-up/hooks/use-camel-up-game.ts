/**
 * Drives a race against the computer from React.
 *
 * @module
 * @remarks
 * The player acts, the computer players answer a beat later so the table can
...
import { applyMove } from "@/games/camel-up/engine/moves";
import { createGame, soloSeats } from "@/games/camel-up/engine/setup";
...
import type { CamelUpSettings } from "@/games/camel-up/settings/app-settings";
...
const GAME_ID: GameId = "camel-up";

/** How long a computer player pauses before its move, so it can be followed. */
...
export type CamelUpSession = {
  readonly game: CamelUpGame;
  /** Which seat the human plays. */
  readonly mySeat: number;
  /** True while a computer player is on turn, so the board stays quiet. */
  readonly busy: boolean;
  readonly play: (move: CamelUpMove) => void;
  /** Deals a fresh game with the settings as they stand. */
  readonly newGame: () => void;
};

/**
 * Runs one race against the computer.
 *
 * @param settings - how many players are in the race
 ...
 */
export function useCamelUpGame(settings: CamelUpSettings): CamelUpSession {
  const [game, setGame] = useState<CamelUpGame>(() =>
  ...
  const beginGame = useCallback((next: CamelUpGame) => {
    const now = Date.now();
  ...
  useEffect(() => {
    if (!isReady.current) {
      ...
      const saved = loadSession(GAME_ID, isCamelUpGame);
      if (saved === null) {
    ...
    if (isReady.current) {
      ...
      if (game.phase === "gameOver" && !meta.current.isOutcomeRecorded) {
        ...
        recordGameFinished(GAME_ID, {
          // The most money wins; a shared lead counts as a win.
  ...
  const play = useCallback((move: CamelUpMove) => {
    setGame((current) => {
      const mine =
