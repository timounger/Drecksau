/**
 * The seeded generator the pyramid draws its dice from.
 *
 * @module
 * @remarks
 * The same generator the other games of the collection use, with its cursor
 * carried **inside** the game state: Camel Up rolls dice from the first turn to
 * the last, so a saved game and a snapshot on the wire both have to remember
 * where the sequence had got to.
 */
