/**
 * Camel Up settings, stored in the browser.
 *
 * @module
 * @remarks
 ...
 */
import { MAX_PLAYERS, MIN_PLAYERS } from "@/games/camel-up/engine/state";
...
const SETTINGS_KEY = storageKey("camel-up", "settings");

/** Table size of a first-time visitor's race. */
...
export type CamelUpSettings = {
  /** How many players bet on the race, the human included. */
  readonly playerCount: number;
};

/** What a first-time visitor gets. */
export const DEFAULT_SETTINGS: CamelUpSettings = {
 ...
 */
export function loadSettings(): CamelUpSettings {
 ...
 */
export function saveSettings(settings: CamelUpSettings): void {
...
function isPartialSettings(value: unknown): value is Partial<CamelUpSettings> {
