/**
 * Flip 7 online: the entry screen, the lobby and the table.
 *
 * @module
 * @remarks
...
import {
  flip7Adapter,
  type Flip7Options,
} from "@/games/flip-7/multiplayer/adapter";
import {
  DEFAULT_PLAYER_COUNT,
  PLAYER_COUNTS,
} from "@/games/flip-7/settings/app-settings";
import {
  loadMatchCount,
  saveMatchCount,
} from "@/games/flip-7/settings/online-settings";
...
const GAME_ID = flip7Adapter.gameId;
 ...
 */
export function Flip7OnlineScreen(): ReactElement {
  ...
  const room = useOnlineRoom(flip7Adapter, session);
...
type Room = OnlineRoom<Flip7Game, Flip7Move, Flip7Options>;
...
function Playing({
  ...
  const remainingMs = useTurnClock({
    ...
    turn: room.room === null ? "" : turnKeyOf(room.room, flip7Adapter),
    running: game !== null && game.stage !== "gameOver",
  });
