/**
 * GTA settings as an external store React can subscribe to.
 *
 * @module
 * @remarks
 * Same reason as the statistics store: localStorage does not exist during the
 * prerender, so the values may only be read after hydration. The game loop
 * subscribes as well, which is what lets a change of the zoom take effect on
 * the next frame rather than the next game.
 */
import {
  DEFAULT_SETTINGS,
  loadSettings,
...
let cache: GtaSettings | null = null;
 ...
 */
export function getSettingsSnapshot(): GtaSettings {
  if (cache === null) {
    cache = loadSettings();
  }
  return cache;
 ...
 */
export function getServerSettingsSnapshot(): GtaSettings {
 ...
 */
export function updateSettings(settings: GtaSettings): void {
