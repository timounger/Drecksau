 ...
 */
import { ONLINE_MIN_PLAYERS } from "@/games/the-game/engine/state";
...
const COUNT_KEY = storageKey("the-game", "online-player-count");
...
export function loadMatchCount(): number {
  ...
  return onlineCount(stored === null ? DEFAULT_PLAYER_COUNT : stored);
}
...
export function saveMatchCount(count: number): void {
  writeStored(COUNT_KEY, ONLINE_VERSION, onlineCount(count));
}

/**
 * Holds a table size to one an online room can actually start.
 *
 * @remarks
 * The solo variant is a real setting for the game against the computer and a
 * contradiction here - a room of one is nobody to play with. So this floor
 * exists on the online side only, rather than being pushed into the shared
 * clamp where it would quietly take the solo game away too.
 */
function onlineCount(value: unknown): number {
  return Math.max(ONLINE_MIN_PLAYERS, clampPlayers(value));
}
