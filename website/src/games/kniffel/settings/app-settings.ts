/**
 * Kniffel settings, stored in the browser.
 *
 * @module
 * @remarks
 ...
 */
import { MAX_PLAYERS, MIN_PLAYERS } from "@/games/kniffel/engine/state";
...
const SETTINGS_KEY = storageKey("kniffel", "settings");
...
/**
 * The table sizes an online room may be wished for.
 *
 * @remarks
 * Kniffel alone is a real way to play - against your own best sheet - so the
 * engine seats one. A room of one is not a room, though, so the online screen
 * asks from two upwards.
 */
export const ONLINE_PLAYER_COUNTS: readonly number[] = PLAYER_COUNTS.filter(
  (count) => count > 1,
);

/** What the player can configure. */
export type KniffelSettings = {
  /** How many fill a sheet, the human included. */
  readonly playerCount: number;
};

/** What a first-time visitor gets. */
export const DEFAULT_SETTINGS: KniffelSettings = {
 ...
 */
export function loadSettings(): KniffelSettings {
 ...
 */
export function saveSettings(settings: KniffelSettings): void {
...
function isPartialSettings(value: unknown): value is Partial<KniffelSettings> {
