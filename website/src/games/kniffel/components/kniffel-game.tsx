/**
 * Kniffel against the computer: the grill, the dice and the log.
 *
 * @module
 * @remarks
 * This is the game's front page - opening `/kniffel` deals a game at once,
...
import {
  ...
  getSettingsSnapshot,
  subscribeSettings,
} from "@/games/kniffel/settings/settings-store";
 ...
 */
export function KniffelGameScreen(): ReactElement {
  const settings = useSyncExternalStore(
    subscribeSettings,
    getSettingsSnapshot,
    getServerSettingsSnapshot,
  );
  const { game, mySeat, play, newGame } = useKniffelGame(settings);
  ...
  return (
    <div className="mx-auto flex w-full max-w-5xl flex-1 flex-col gap-4 p-4">
      <GameHeader rules={KNIFFEL_RULES} title={T.title} subtitle={T.tagline}>
        ...
        <NavLink href="/kniffel/online">{T.playOnline}</NavLink>
        <NavLink href="/kniffel/einstellungen">{T.settings}</NavLink>
        <NavLink href="/kniffel/statistik">{T.statistics}</NavLink>
      </GameHeader>

      {game.phase === "gameOver" && (
        <KniffelScores game={game} onNewGame={newGame} />
      )}

      <KniffelTable game={game} mySeat={mySeat} onMove={play} />
