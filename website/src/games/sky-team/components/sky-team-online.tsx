...
import {
  skyTeamAdapter,
  type SkyTeamOptions,
} from "@/games/sky-team/multiplayer/adapter";
import {} from "@/games/sky-team/settings/app-settings";

import { SKY_TEAM_TEXTS as T } from "@/games/sky-team/i18n/texts";
import { loadPlayerName, savePlayerName } from "@/online/player-name";
import { SkyTeamScores } from "./sky-team-scores";
import { SkyTeamCockpit } from "./sky-team-cockpit";
...
/** Always two - the box says so, and there is nothing to choose. */
const SEATS = 2;
...
const GAME_ID = skyTeamAdapter.gameId;
 ...
 */
export function SkyTeamOnlineScreen(): ReactElement {
  ...
  const room = useOnlineRoom(skyTeamAdapter, session);
    ...
    <div className="mx-auto flex w-full max-w-6xl flex-1 flex-col gap-4 p-4">
      <header className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <RulesButton rules={SKY_TEAM_RULES} />
          <Link
            href="/sky-team"
...
type Room = OnlineRoom<SkyTeamGame, SkyTeamMove, SkyTeamOptions>;
  ...
  return game === null ? (
    ...
    <div className="flex flex-col gap-4 lg:flex-row">
      ...
      <div className="flex flex-1 flex-col gap-4">
        ...
        {/* Somebody has to press on once the eighth die is down. The engine
            names the seat that opens the next round, so it is theirs - and
            without this the table simply sat there with a full cockpit. */}
        {game.stage === "roundEnd" && mySeat === game.opener && (
          <button
            type="button"
            data-testid="sky-next"
            onClick={() => room.sendMove({ kind: "next" })}
