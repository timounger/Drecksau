/**
 * The seeded generator the dice are thrown from.
 *
 * @module
 * @remarks
 * The same generator the other games of the collection use, with its cursor
 * carried **inside** the game state. Every round is a fresh throw from the same
 * sequence, so a saved landing and a snapshot on the wire both have to remember
 * where that sequence had got to - otherwise a reload would re-roll the dice,
 * and in this game that is the difference between a landing and a crash.
 */
