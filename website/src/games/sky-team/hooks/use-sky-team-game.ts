...
import { aiMove, botWaitMs } from "@/games/sky-team/engine/ai";
...
import { createGame, soloSeats } from "@/games/sky-team/engine/setup";
...
import type { SkyTeamSettings } from "@/games/sky-team/settings/app-settings";
...
const GAME_ID: GameId = "sky-team";

/** The seat the human flies, from the settings. */
function humanSeat(asPilot: boolean): Seat {
  return asPilot ? 0 : 1;
}
...
export type SkyTeamSession = {
  readonly game: SkyTeamGame;
  /** Which seat the human flies. */
  readonly mySeat: Seat;
  readonly play: (move: SkyTeamMove) => void;
 ...
 */
export function useSkyTeamGame(settings: SkyTeamSettings): SkyTeamSession {
  const [game, setGame] = useState<SkyTeamGame>(() =>
    createGame(soloSeats(HUMAN_NAME, true), INITIAL_SEED),
  );
  ...
  const beginGame = useCallback((next: SkyTeamGame) => {
    const now = Date.now();
  ...
  useEffect(() => {
    if (!isReady.current) {
      ...
      const saved = loadSession(GAME_ID, isSkyTeamGame);
      if (saved === null) {
        const fresh = createGame(
          soloSeats(HUMAN_NAME, settingsRef.current.asPilot),
          Date.now() >>> 0,
        );
        beginGame(fresh);
        setGame(fresh);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      setGame((current) => {
        const seat = pending as Seat;
        const move = aiMove(current, seat);
        return (
          (move === null ? null : applyMove(current, seat, move)) ?? current
        );
      });
    }, botWaitMs());
  ...
  const play = useCallback((move: SkyTeamMove) => {
    setGame(
      (current) =>
        applyMove(current, humanSeat(settingsRef.current.asPilot), move) ??
        current,
    );
  }, []);
  ...
  return { game, mySeat: humanSeat(settings.asPilot), play, newGame };
}
