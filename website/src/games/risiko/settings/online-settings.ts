 ...
 */
import { MIN_PLAYERS } from "@/games/risiko/engine/state";
import { readStored, storageKey, writeStored } from "@/lib/storage/local-store";
import { DEFAULT_PLAYER_COUNT, clampPlayers } from "./app-settings";
...
const COUNT_KEY = storageKey("risiko", "online-player-count");
 ...
 *
 * @remarks
 * Two is a real answer here, and it means the two-player variant with its
 * neutral armies rather than a short basic game - the box has no basic game for
 * two. Everything above two is the basic game's three to five.
 */
function onlineCount(value: unknown): number {
  const wanted =
    typeof value === "number" && Number.isFinite(value) ? Math.round(value) : 0;
  return wanted === MIN_PLAYERS
    ? MIN_PLAYERS
    : clampPlayers(wanted, "grundspiel");
}
