/**
 * Kuhle Kühe online: the entry screen, the lobby and the table.
 *
 * @module
 * @remarks
...
import {
  kuhleKueheAdapter,
  type KuhleKueheOptions,
} from "@/games/kuhle-kuehe/multiplayer/adapter";
import {
  DEFAULT_PLAYER_COUNT,
  PLAYER_COUNTS,
} from "@/games/kuhle-kuehe/settings/app-settings";
import {
  loadMatchCount,
  saveMatchCount,
} from "@/games/kuhle-kuehe/settings/online-settings";
...
const L = {
  ...
  subtitle: "Ein Wurf, viele Zettel - jede:r am eigenen Gerät",
  back: "Zurück",
  yourName: "Dein Name",
...
const GAME_ID = kuhleKueheAdapter.gameId;
 ...
 */
export function KuhleKueheOnlineScreen(): ReactElement {
  ...
  const room = useOnlineRoom(kuhleKueheAdapter, session);
...
type Room = OnlineRoom<KuhleKueheGame, KuhleKueheMove, KuhleKueheOptions>;
...
function Playing({
  ...
  const remainingMs = useTurnClock({
    ...
    turn: room.room === null ? "" : turnKeyOf(room.room, kuhleKueheAdapter),
