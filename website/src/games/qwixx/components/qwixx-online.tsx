...
import { QWIXX_RULES } from "@/games/qwixx/i18n/rules";
import {
  useCallback,
  useEffect,
  useRef,
...
import {
  qwixxAdapter,
  type QwixxOptions,
} from "@/games/qwixx/multiplayer/adapter";
import {
  DEFAULT_PLAYER_COUNT,
  PLAYER_COUNTS,
} from "@/games/qwixx/settings/app-settings";
import {
  loadMatchCount,
  saveMatchCount,
} from "@/games/qwixx/settings/online-settings";
...
const GAME_ID = qwixxAdapter.gameId;
 ...
 */
export function QwixxOnlineScreen(): ReactElement {
  ...
  const room = useOnlineRoom(qwixxAdapter, session);
...
type Room = OnlineRoom<QwixxGame, QwixxMove, QwixxOptions>;
...
function Playing({
  ...
  const remainingMs = useTurnClock({
    ...
    turn: room.room === null ? "" : turnKeyOf(room.room, qwixxAdapter),
