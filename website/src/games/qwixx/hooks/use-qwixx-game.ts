...
import { applyMove } from "@/games/qwixx/engine/moves";
import { isQwixxGame } from "@/games/qwixx/engine/serialization";
import { createGame, soloSeats } from "@/games/qwixx/engine/setup";
import {
  leaders,
  type QwixxGame,
  type QwixxMove,
} from "@/games/qwixx/engine/state";
import type { QwixxSettings } from "@/games/qwixx/settings/app-settings";
...
export type QwixxSession = {
  readonly game: QwixxGame;
  /** Which seat the human plays. */
  readonly mySeat: number;
  readonly play: (move: QwixxMove) => void;
 ...
 */
export function useQwixxGame(settings: QwixxSettings): QwixxSession {
  const [game, setGame] = useState<QwixxGame>(() =>
  ...
  const beginGame = useCallback((next: QwixxGame) => {
    const now = Date.now();
  ...
  useEffect(() => {
    if (!isReady.current) {
      ...
      const saved = loadSession(GAME_ID, isQwixxGame);
      if (saved === null) {
  ...
  /**
   * The next computer seat that owes an answer, or -1.
   *
   * @remarks
   * In the white step that is any computer player who has not decided yet - a
   * turn can therefore need several computer moves before it is the human's
   * problem again.
   */
  const pending =
    game.phase === "gameOver"
      ...
      : game.phase === "white"
        ? game.players.findIndex(
            (player, seat) => player.isBot && !game.decided[seat],
          )
        : game.players[game.active].isBot
          ? game.active
          : -1;
  ...
  const play = useCallback((move: QwixxMove) => {
