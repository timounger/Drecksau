...
function seven(game: CatanGame): CatanGame {
  ...
  const after: Phase = robberLoose(game) ? "robber" : "trade";
...
function doRoll(game: CatanGame): CatanGame {
  ...
  if (playing(game, "ereignisse")) {
    ...
    next =
      rolled === ROBBER_ROLL
        ? seven(after)
        : afterYield(produce(after, rolled));
  }
  return next;
}
...
function afterYield(game: CatanGame): CatanGame {
  return { ...game, phase: owesRoll(game) ? "roll" : "trade" };
}
...
function doDiscard(
  ...
  // The robber is still nailed down until the barbarians have landed once.
  const after: Phase = robberLoose(game) ? "robber" : "trade";
  return ok
    ? note(
        {
          ...
          owing: rest,
          phase: rest.length === 0 ? after : "discard",
        },
...
function doTown(game: CatanGame, seat: number, at: number): CatanGame | null {
  ...
  if (allowed) {
    ...
    const paidChips = withChips(charged, seat, chipsForTown(charged, at));
    next = owedNeutral(
      checkWinner(
        awardTiles(
          note(paidChips, `${nameOf(game, seat)}: baut eine Siedlung.`),
        ),
      ),
      seat,
      "town",
...
function doFounding(
  ...
  if (
    ...
    const raised =
      built !== null && second && playingRitter(game)
    ...
    next =
      raised === null
        ? null
        : {
            ...(second ? firstIncome(raised, seat, move.at) : raised),
...
function doProgress(
  ...
  const timely =
    card === "alchemie" ? game.phase === "roll" : game.phase === "trade";
  let next: CatanGame | null = null;
...
function applyTurnMove(
  ...
  const handlers: Readonly<Record<string, () => CatanGame | null>> = {
    ...
    endTurn: () =>
      // The turn is not over while a free neutral piece or two cards are owed.
      rolled && game.neutralBuild === null && game.swapWith === null
        ? nextTurn(game)
        : null,
  };
