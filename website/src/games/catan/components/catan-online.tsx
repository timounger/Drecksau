/**
 * CATAN online: the entry screen, the lobby and the board.
 *
 * @module
 * @remarks
 ...
 * it hands back.
 *
 * **Catan is the game in this collection that most wants other people.** The
 * computer builds sensibly and will take a trade that is clearly good for it,
 * but the whole middle of Catan is talking somebody into a trade that is not
 * quite - and no bot falls for that. Which is why the voice and text chat
 * matter here as much as the board does.
 *
 * A turn is long and can be interrupted by things that are not the active
 * player's at all: a seven puts everybody with a full hand into a discard
 * queue, and an offer puts everybody into an answering one. The auto-play clock
 * counts each of those separately, so somebody deciding what to throw away is
 * not racing a clock that belongs to the player whose turn it is. See the
 * adapter's `turnKey`.
 */
"use client";
...
import {
  catanAdapter,
  type CatanOptions,
} from "@/games/catan/multiplayer/adapter";
import {
  DEFAULT_PLAYER_COUNT,
  PLAYER_COUNTS,
} from "@/games/catan/settings/app-settings";
import {
  loadMatchCount,
  saveMatchCount,
...
import { CatanCards, CatanHand, CatanStandings } from "./catan-panels";
...
const L = {
  ...
  subtitle: "Siedeln, handeln, bauen - jede:r am eigenen Gerät",
  back: "Zurück",
  yourName: "Dein Name",
...
const GAME_ID = catanAdapter.gameId;

/**
 * How long a player may dither before the computer plays their turn.
 *
 * @remarks
 * Two minutes, which is a lot and is right: a Catan turn can be a roll, a
 ...
 */
export function CatanOnlineScreen(): ReactElement {
  ...
  const room = useOnlineRoom(catanAdapter, session);
...
type Room = OnlineRoom<CatanGame, CatanMove, CatanOptions>;
  ...
  return game === null ? (
    ...
    <div className="flex flex-col gap-4 lg:flex-row">
        ...
        <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-zinc-200 bg-white p-3 text-sm dark:border-zinc-800 dark:bg-zinc-900">
          ...
          {mySeat >= 0 && (
            <span className="text-zinc-500 dark:text-zinc-400">
              {T.points(openPoints(game, mySeat))}
            </span>
          )}
      ...
      <aside className="flex w-full flex-col gap-3 lg:w-96">
        <CatanStandings game={game} mySeat={mySeat >= 0 ? mySeat : null} />
        {mySeat >= 0 && game.phase !== "gameOver" && (
          <>
