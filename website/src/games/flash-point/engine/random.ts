/**
 * The seeded generator the fire is rolled from.
 *
 * @module
 * @remarks
 * The same generator the other games of the collection use, with its cursor
 * carried **inside** the game state. Every turn rolls the fire's next square from the
 * same sequence, so a saved fire and a snapshot on the wire both have to
 * remember where that sequence had got to - otherwise a reload would re-roll,
 * and the house would burn somewhere else than it did a moment ago.
 */
