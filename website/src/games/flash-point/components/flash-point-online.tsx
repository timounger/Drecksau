/**
 * The Mind online: the entry screen, the lobby and the table.
 *
 * @module
 * @remarks
 * The game is turn-based - even the votes go round the table one seat at a time
 * rather than all at once - so it runs on the shared online layer rather than a
 * loop of its own: {@link useOnlineRoom} does the room, the host election and
 * the redaction, and this screen only renders what it hands back.
 *
...
import { MAX_PLAYERS } from "@/games/flash-point/engine/setup";
import { ONLINE_MIN_PLAYERS as MIN_PLAYERS } from "@/games/flash-point/multiplayer/adapter";
import {
  TOTAL_CUBES,
  type FlashPointGame,
  type FlashPointMove,
...
import {
  flashPointAdapter,
  type FlashPointOptions,
} from "@/games/flash-point/multiplayer/adapter";
...
import { FlashPointScores } from "./flash-point-scores";
import { FlashPointBoard } from "./flash-point-board";
...
/**
 * The crew sizes an online room offers.
 *
 * @remarks
 * Derived from what the game can seat rather than written out, so the two
 * cannot drift apart when the engine's limits move.
 */
const CREW: readonly number[] = Array.from(
  { length: MAX_PLAYERS - MIN_PLAYERS + 1 },
  (unused, index) => MIN_PLAYERS + index,
);

/** What a room waits for when nobody says otherwise. */
const DEFAULT_CREW = MIN_PLAYERS + 1;
...
const L = {
  ...
  subtitle: "Gemeinsam schweigen, jede:r am eigenen Gerät",
  back: "Zurück",
  yourName: "Dein Name",
  namePlaceholder: "z. B. Alex",
  crewLabel: "Mannschaft:",
...
const GAME_ID = flashPointAdapter.gameId;

/**
 * How long a player may dither before the computer plays their turn.
 *
 * @remarks
 * A turn is one of four things and none of them takes long to decide, so this
 * is on the short side: the whole appeal of the game is that it moves, and a
 * table waiting a minute for somebody to pick a camel is not that game.
 */
 ...
 */
export function FlashPointOnlineScreen(): ReactElement {
  ...
  const [wanted, setWanted] = useState(DEFAULT_CREW);
  const room = useOnlineRoom(flashPointAdapter, session);
    ...
    <div className="mx-auto flex w-full max-w-6xl flex-1 flex-col gap-4 p-4">
      <header className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <RulesButton rules={FLASH_POINT_RULES} />
          <Link
            href="/flash-point"
...
type Room = OnlineRoom<FlashPointGame, FlashPointMove, FlashPointOptions>;
...
function Entry({
  ...
  // Always two. Kept as a constant rather than a state, because there is
  // nothing to change it to.
  const [count, setCount] = useState(DEFAULT_CREW);
  ...
  return (
    <div className="flex max-w-md flex-col gap-5">
      ...
      <div className="flex flex-col gap-3 rounded-2xl border border-emerald-200 bg-emerald-50/40 p-4 dark:border-emerald-900/50 dark:bg-emerald-950/20">
        ...
        {/* How big a crew to wait for. Every extra pair of hands is four more
            action points a round - and one more turn before the fire rolls
            again, which is not always a gift. */}
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          {CREW.map((size) => (
            <button
              key={size}
              type="button"
              data-testid={`fp-count-${size}`}
              onClick={() => setCount(size)}
              className={`cursor-pointer rounded-lg border px-3 py-1 text-sm font-semibold ${
                size === count
...
function Playing({
  ...
  const outcome: GameOutcome | null =
    game === null || (game.stage !== "won" && game.stage !== "lost")
      ? null
      : game.stage === "won"
        ? "won"
        : "lost";
  ...
  return game === null ? (
    ...
    <div className="flex flex-col gap-4 lg:flex-row">
      ...
      <div className="flex flex-1 flex-col gap-4">
        ...
        {/* Offline the screen above the board says whose turn it is. Online
            that is the one thing nobody can see for themselves, so it has to be
            here too - a cooperative game where you cannot tell whether you are
            waiting or holding everybody up is no fun at all. */}
        <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-zinc-200 bg-white p-3 text-sm dark:border-zinc-800 dark:bg-zinc-900">
          ...
          <span className="text-zinc-500 dark:text-zinc-400">
            {T.apLeft}: {game.players[game.active].ap}
          </span>
          <span className="text-zinc-500 dark:text-zinc-400">
        ...
        />
        {/* Online this is not decoration. Between your turns the fire has had
            several goes, and the log is the only place that says what it did
            while you were watching somebody else. */}
