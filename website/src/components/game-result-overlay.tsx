/**
 * The big end-of-game animation: a trophy for a win, dung for a loss.
 *
 * @module
 * @remarks
 * Plays once when the game ends, longer and larger than anything else on the
 * screen, then fades on its own so the result panel and buttons underneath stay
 * usable. It never takes a click.
 *
 * Shared rather than copied. Two games showing "the same" finale from two
 * files are the same finale only until somebody edits one of them, and the
 * length is exactly the sort of number that drifts unnoticed.
 *
 * What a game brings is its own word for winning and losing, and whether its
 * animation setting allows this at all.
 */
"use client";
...
/** Look of each outcome: the centre icon, the raining icon and the tint. */
...
export type GameResultOverlayProps = {
  ...
  /** The big word each outcome puts on the screen, in the game's own voice. */
  readonly words: Readonly<Record<GameOutcome, string>>;
  /**
   * Whether the game's animation setting allows this.
   *
   * @remarks
   * Left out by a game that has no such setting. Switching animations off has
   * to be a real choice a player made, which is why it is decided here and not
   * by a media query in the stylesheet.
   */
 ...
 *
 * @param props - the outcome, the words for it and the animation setting
    ...
    overlay = (
      ...
      >
        <span
          ...
          style={{
            animation: `game-result-tint ${GAME_OVER_DURATION_MS}ms ease-out both`,
          }}
        />
        ...
        {range(FALL_COUNT).map((index) => (
          <span
            ...
            style={{
              ...
              animation: `game-fall ${FALL_MIN_MS + (index % FALL_VARIANTS) * FALL_STEP_MS}ms linear ${(index % FALL_DELAY_VARIANTS) * FALL_DELAY_STEP_MS}ms infinite`,
            }}
          >
            {look.fallIcon}
          </span>
        ...
        <div
          ...
          style={{
            animation: `game-result ${GAME_OVER_DURATION_MS}ms ease-out both`,
          }}
        >
          <span className="text-[7rem] leading-none sm:text-[9rem]">
            {look.icon}
          </span>
          <span className="text-6xl font-black tracking-tight text-amber-100 sm:text-8xl">
            {words[shown.outcome]}
          </span>
        </div>
      </div>
