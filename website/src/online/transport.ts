/**
 * The seam between the room logic and the network, generic in the game.
 *
 * @module
 * @remarks
 * The room reducer ({@link ./room}) is pure and knows nothing about Firebase.
 * A transport carries the traffic between the players; Firebase implements it
 * first, but a peer-to-peer or self-hosted transport could implement the same
 * interface later without touching any game.
 *
...
/** A move a guest asks the host to make on their behalf, generic in the move. */
export type MoveIntent<M> = {
...
export type RoomTransport<G, M, H> = {
  /**
   ...
   */
  publish(shared: RoomState<G>, hands: ReadonlyMap<SeatId, H>): Promise<void>;
   ...
   * @returns an unsubscribe function
   */
  onShared(onShared: (shared: RoomState<G>) => void): () => void;
   ...
   * @returns an unsubscribe function
   */
  onHand(seatId: SeatId, onHand: (hand: H) => void): () => void;
   ...
   */
  sendIntent(intent: MoveIntent<M>): Promise<void>;
   ...
   * @returns an unsubscribe function
   */
  onIntents(onIntent: (intent: MoveIntent<M>) => void): () => void;
   ...
   */
  readHands(): Promise<ReadonlyMap<SeatId, H>>;

  /** Leaves the room and releases every listener and the connection. */
  disconnect(): Promise<void>;
};

/** The untrusted-value guards a transport needs to filter wire data. */
export type WireGuards<G, M, H> = {
  isRoomState(value: unknown): value is RoomState<G>;
  isMoveIntent(value: unknown): value is MoveIntent<M>;
